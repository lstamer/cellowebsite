...
Supabase is the operational source of truth for every enquiry: WhatsApp
conversations here, and the website forms via `inquiry_website_leads`. The
admin area at `admin.stamer.co.za` is the CRM view over it (plans/007).
...
`inquiry_website_leads` (required: a Supabase failure returns 500 to the
form and logs an `admin_events` row; the Telegram alert is best-effort,
tracked in `alert_status` and retried by the `retry-lead-alerts` task). When
the lead has a
