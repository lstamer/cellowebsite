...
Luke Stamer is a solo cellist in Cape Town (site: https://stamer.co.za). All booking
...
| Layer | Tech | Details |
|---|---|---|
| Website / webhook host | Next.js (App Router) on Vercel | Project `maestro-stamers-projects/cellowebsite`, prod domain **https://stamer.co.za** (apex ONLY — www 307-redirects and `*.vercel.app` is blocked by Deployment Protection; webhooks must use the apex) |
...
7. **Production E2E smoke**: signed synthetic inquiry → Supabase rows → Trigger run → AI
   ...
   → synthetic rows cleaned up. ⬅ current step
8. Reviewer (architect) verification, anti-slop pass on changed files, regression re-run. 
9. Real-phone smoke (Luke sends 3 bubbles, approves once) — final human acceptance.

## 6. Current status (update me!)

**Last updated: 2026-07-12 (Ralph iteration 3, session 78c3e5e6)**

- Stories US-001…US-006 and US-008 verified ✅ (see `progress.txt` for evidence trail).
- The three previously permission-gated actions were run by Luke and re-verified:
  Vercel env fixed (`SUPABASE_URL` → qrefdgmn…, `ZERNIO_WEBHOOK_SECRET` added, redeployed —
  zernio route now returns designed 401), Trigger tasks deployed (`dispatch-inquiry-outbox`
  cron active).
- Production smoke test (US-007) in progress via `scripts/smoke-inquiry.mjs`.
- Remaining after smoke: architect review → deslop → regression → close Ralph loop; then
  real-phone acceptance test with Luke.
