  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Link href={adminPath(open ? "/console?open=0" : "/console")} className="inline-flex min-h-11 items-center rounded-full border border-foreground/15 px-[1.25em] py-[0.6em] font-sans text-sm text-foreground hover:border-foreground/40">
        ...
        {attention.length === 0 ? (
          ...
          <ul className="flex flex-col divide-y divide-foreground/10">
            {attention.map((row) => {
              ...
              const body = (
                <>
                  <span className="font-sans text-sm font-medium text-foreground">{row.title}</span>
                  {row.detail ? <span className="mt-0.5 block font-sans text-xs text-foreground/60">{row.detail}</span> : null}
                  <span className="mt-0.5 block font-jost text-[0.6875rem] uppercase tracking-[0.14em] text-foreground/50">
              ...
              return (
                <li key={`${row.kind}-${row.entity_id}`} className="py-3">
                  {href ? <Link href={href} className="block hover:text-primary">{body}</Link> : <div>{body}</div>}
                </li>
              );
            })}
          </ul>
              ...
              <li key={event.id} className="flex flex-col gap-2 py-3 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="font-jost text-[0.6875rem] uppercase tracking-[0.14em] text-foreground/50">
                      {event.source} · {event.kind.replace(/_/g, " ")}
                    </span>
                    <span className="font-sans text-xs text-foreground/50">{formatDateTime(event.created_at)}</span>
                  </div>
                  <p className="mt-1 break-words font-sans text-sm text-foreground/85">{event.message}</p>
                  {event.entity_type === "website_lead" && event.entity_id ? (
                    <Link href={adminPath(`/inquiries/${event.entity_id}`)} className="font-sans text-xs text-foreground/60 underline-offset-4 hover:underline">
                      Open lead
                    </Link>
                  ) : null}
                  {Object.keys(event.context).length > 0 ? (
                    <details className="mt-1">
                      <summary className="cursor-pointer font-sans text-xs text-foreground/50">Context</summary>
                      <pre className="mt-1 overflow-x-auto rounded-input bg-cream p-2 font-mono text-xs text-foreground/70">{JSON.stringify(event.context, null, 2)}</pre>
                    </details>
                  ) : null}
                  {event.acknowledged_at ? (
                    <p className="mt-1 font-sans text-xs text-foreground/50">Acknowledged by {event.acknowledged_by} · {formatRelative(event.acknowledged_at)}</p>
                  ) : null}
                </div>
                {!event.acknowledged_at ? (
